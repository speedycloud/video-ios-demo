...
- (void)addSpecifiedItem:(NSDictionary *)dic
{
	...
	[dataDic setObject:@"songwei" forKey:@"strName"];
...
- (NSDictionary *)getDic
{
    ...
    int randomNum = arc4random()%1;
    if (randomNum == UUMessageTypePicture) {
        [dictionary setObject:[UIImage imageNamed:[NSString stringWithFormat:@"%zd.jpeg",arc4random()%5]] forKey:@"picture"];
    } else {
...
- (NSString *)getName:(NSInteger)index{
    NSArray *array = @[@"xunda",@"songwei",@"gaoyanbo",@"Hey,Bob",@"Hah,Dane",@"Wow,Boss"];
    return array[index];
}
@end
